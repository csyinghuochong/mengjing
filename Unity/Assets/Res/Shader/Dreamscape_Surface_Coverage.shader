	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				float2 uv_CoverageNormalTexture = IN.ase_texcoord8.xy * _CoverageNormalTexture_ST.xy + _CoverageNormalTexture_ST.zw;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float4 temp_output_5_0 = ( _AlbedoTint * tex2D( _AlbedoTexture, uv_AlbedoTexture ) );
				float2 uv_CoverageTexture = IN.ase_texcoord4.xy * _CoverageTexture_ST.xy + _CoverageTexture_ST.zw;
		...
		Pass
		{
						 ...
						 ) : SV_TARGET
			{
				...
				float3 tex2DNode76 = UnpackNormalScale( tex2D( _NormalTexture, uv_NormalTexture ), 1.0f );
				float2 uv_CoverageNormalTexture = IN.ase_texcoord4.xy * _CoverageNormalTexture_ST.xy + _CoverageNormalTexture_ST.zw;
				float3 tex2DNode79 = UnpackNormalScale( tex2D( _CoverageNormalTexture, uv_CoverageNormalTexture ), 1.0f );
				#ifdef _BLENDNORMALSON_ON
				float3 staticSwitch80 = BlendNormal( tex2DNode76 , tex2DNode79 );
				#else
...
ASEEND*/
//CHKSM=B5952DF5498728981B9522B77434C1439BEF5567