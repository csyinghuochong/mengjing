...
public class TextFitTip : MonoBehaviour
{
    ...
    IEnumerator ClearUpPunctuationMode()
    {
        ...
        bool debuglog = false;
        if (this.targetText.text.Contains(("禁止使用全部或部分功能")))
        {
            debuglog = true;
        ...
        for (int i = 1; i < TextLine.Count; i++)
        {
            ...
            if (i < TextLine.Count - 1)
            {
                int startchatindex = TextLine[i].startCharIdx;
                int endchatindex = TextLine[i + 1].startCharIdx - 1;
        ...
        isProcessingText = false;
        if (havahuanhang || this.updateNumber > 0)
        {
            this.OnTextChange();
            this.updateNumber--;
        }
