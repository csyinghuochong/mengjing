...
namespace ET.Client
{
    public static class UILoginHelper
    {
        ...
        public static async ETTask ShowTextList(Scene root, GameObject nodeparent, GameObject textItem, int platForm)
        {
        ...
        public static string GetYingSiText(int platform)
        {
            try
            {
                WebClient MyWebClient = new WebClient();
                MyWebClient.Credentials = CredentialCache.DefaultCredentials; //获取或设置用于向Internet资源的请求进行身份验证的网络凭据
                //string dataurl = (platform == 5 || platform == 6) ? "http://verification.weijinggame.com/weijing/yinsi3.txt"
                 //       : "http://verification.weijinggame.com/weijing/yinsi1.txt";
                string dataurl = "http://verification.weijinggame.com/weijing/yinsi5.txt";
