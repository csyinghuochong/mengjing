...
/// 避免了图形重建循环和无限处理循环
/// </summary>
public class ImprovedTextEx : Text
{
    /// <summary>
    /// 用于匹配常见的中文标点符号（正则表达式）
    /// 可根据需要增删标点
    ...
    private void ProcessTextLayout()
    {
        ...
        try
        {
            ...
            for (int i = 1; i < lines.Count; i++)
            {
                ...
                string firstchat =  processedText[lineStartIndex].ToString();
                
                // 检查行首字符是否为标点
                if (Regex.IsMatch(firstchat, punctuationRegex) || punctuationRegex.Contains((firstchat)))
                {
                    ...
                    if (prevLineEndIndex > 0 && prevLineEndIndex < processedText.Length - 1)
                    {
                        ...
                        {
                            // 在合适位置插入换行符，强制提前换行
                            processedText.Insert(prevLineEndIndex , '\n');
                            textModified = true;
    ...
    private int FindPreviousLineEnd(StringBuilder text, int currentLineStart)
    {
        ...
        while (lineStart > 0  )
        {
            char ccc = text[lineStart - 1];
            if (char.IsWhiteSpace(ccc) && ccc!='\n')
            {
                lineStart--;
            }
            else
