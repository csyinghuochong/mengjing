...
namespace ET
{
    public static class PetHelper
    {
        ...
        public static Dictionary<int, int> GetEquipSkillList(RolePetInfo rolePetInfo, List<BagInfo> bagComponent)
        {
            ...
            for (int i = 0; i < rolePetInfo.PetEquipList.Count; i++)
            {
                ...
                for (int bag = 0; bag < bagComponent.Count; bag++)
                {
                    if (bagComponent[bag].BagInfoID == baginfoId)
                    {
                        bagInfo = bagComponent[bag];
                        break;
                    }
                }
        ...
        public static (RolePetInfo, RolePetInfo) GetPetHeChengZiZhiPreview(RolePetInfo rolePetA, RolePetInfo rolePetB)
        {
            ...
            rolePetInfoMin.ZiZhi_Hp = Math.Min((int)(Math.Min(zizhiNow_Hp_1, zizhiNow_Hp_2) * 0.95f),3200);
            rolePetInfoMin.ZiZhi_Act = Math.Min((int)(Math.Min(zizhiNow_Act_1, zizhiNow_Act_2) * 0.95f), 1600);
            rolePetInfoMin.ZiZhi_MageAct = Math.Min((int)(Math.Min(zizhiNow_MageAct_1, zizhiNow_MageAct_2) * 0.95f), 1600);
            rolePetInfoMin.ZiZhi_Def = Math.Min((int)(Math.Min(zizhiNow_Def_1, zizhiNow_Def_2) * 0.95f), 1600);
            rolePetInfoMin.ZiZhi_Adf = Math.Min((int)(Math.Min(zizhiNow_Adf_1, zizhiNow_Adf_2) * 0.95f), 1600);
            rolePetInfoMin.ZiZhi_ActSpeed = Math.Min((int)(Math.Min(zizhiNow_ActSpeed_1, zizhiNow_ActSpeed_2) * 0.95f), 1600);
            rolePetInfoMin.ZiZhi_ChengZhang = (float)(Math.Min(zizhiNow_ChengZhang_1, zizhiNow_ChengZhang_2) * 0.95f);

            rolePetInfoMax.ZiZhi_Hp = Math.Min((int)(Math.Max(zizhiNow_Hp_1, zizhiNow_Hp_2) * 1.05f), 3200);
            rolePetInfoMax.ZiZhi_Act = Math.Min((int)(Math.Max(zizhiNow_Act_1, zizhiNow_Act_2) * 1.05f), 1600);
            rolePetInfoMax.ZiZhi_MageAct = Math.Min((int)(Math.Max(zizhiNow_MageAct_1, zizhiNow_MageAct_2) * 1.05f), 1600);
            rolePetInfoMax.ZiZhi_Def = Math.Min((int)(Math.Max(zizhiNow_Def_1, zizhiNow_Def_2) * 1.05f), 1600);
            rolePetInfoMax.ZiZhi_Adf = Math.Min((int)(Math.Max(zizhiNow_Adf_1, zizhiNow_Adf_2) * 1.05f), 1600);
            rolePetInfoMax.ZiZhi_ActSpeed = Math.Min((int)(Math.Max(zizhiNow_ActSpeed_1, zizhiNow_ActSpeed_2) * 1.05f), 1600);
            rolePetInfoMax.ZiZhi_ChengZhang = (float)(Math.Max(zizhiNow_ChengZhang_1, zizhiNow_ChengZhang_2) * 1.05f);
        ...
        public static int GetPetMaxNumber(Unit unit, int level, int petExtendNumber)
        {
            ...
            for (int i = 0; i < petInfos.Length; i++)
            {
                ...
                {
                    return petNumber + petExtendNumber;
                }
            }
