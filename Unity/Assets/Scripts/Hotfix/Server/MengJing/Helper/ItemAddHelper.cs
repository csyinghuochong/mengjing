...
namespace ET.Server
{
    ...
    public static class ItemAddHelper
    {
        ...
        public static void TreasureItem(Unit unit, BagInfo bagInfo)
        {
            ...
            Log.Debug($"生成藏宝图:  {unit.Id} {unit.GetComponent<UserInfoComponentServer>().GetName()} {rewardList[0].ItemID}");
        }
        ...
        public static List<HideProList> GetEquipZhuanJingHidePro(int equipID, int itemID, int jianDingPinZhi, Unit unit, bool ifItem)
        {
            ...
            */
            ...
            if (ifItem)
            {
                if (randomNum >= 2)
                {
                    string noticeContent = $"恭喜玩家<color=#B6FF00>{unit.GetComponent<UserInfoComponentServer>().GetName()}</color>使用鉴定符鉴定装备时,一道金光装备出现<color=#FFA313>{randomNum}条极品属性</color>";
