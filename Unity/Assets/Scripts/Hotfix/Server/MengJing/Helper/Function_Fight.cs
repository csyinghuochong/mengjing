...
namespace ET.Server
{
    ...
    public static class Function_Fight
    {
        ...
        public static bool Fight(Unit attackUnit, Unit defendUnit, SkillHandler skillHandler, int hurtMode)
        {
            ...
            //已死亡
            if (defendUnit.GetComponent<NumericComponentServer>().GetAsInt(NumericType.Now_Dead) == 1)
            {
                return false;
            }
            //无敌buff，不受伤害
            ...
            {
                //攻击强度和法术强度
                switch (attackUnit.GetComponent<UserInfoComponentServer>().UserInfo.Occ)
                {
                    //战士
                    case 1:
            ...
            switch (defendUnit.Type)
            {
                ...
                case UnitType.Player:
                    defendUnitLv = defendUnit.GetComponent<UserInfoComponentServer>().UserInfo.Lv;
                    //受击增加怒气值
            ...
            switch (attackUnit.Type)
            {
                ...
                //玩家
                case UnitType.Player:
                    attackUnitLv = attackUnit.GetComponent<UserInfoComponentServer>().UserInfo.Lv;
            ...
            if (ifHit)
            {
                ...
                {
                    //战士降低受到怪物普攻20%的伤害
                    if (defendUnit.GetComponent<UserInfoComponentServer>().UserInfo.Occ == 1)
                    {
                ...
                if (playerPKStatus)
                {
                    ...
                    else
                    {
                        ...
                        if (attackUnit.Type == UnitType.Player)
                        {
                            int occtwo = attackUnit.GetComponent<UserInfoComponentServer>().UserInfo.OccTwo;
                            if (occtwo != 0)
                            {
        ...
        public static void UnitUpdateProperty_Base(Unit unit, bool notice, bool rank)
        {
            ...
            List<BagInfo> equipList = unit.GetComponent<BagComponentServer>().GetItemByLoc(ItemLocType.ItemLocEquip);
            ...
            for (int i = equipList.Count - 1; i >= 0; i--)
            {
                ...
                int occTwoValue = unit.GetComponent<UserInfoComponentServer>().UserInfo.OccTwo;
                if (occTwoValue != 0)
                {
            ...
            for (int i = 0; i < equipSuitIDList.Count; i++)
            {
                ...
                else  //时装套装
                {
                    ...
                    for (int y = 0; y < needEquipList.Length; y++)
                    {
                        if (unit.GetComponent<BagComponentServer>().FashionActiveIds.Contains(needEquipList[y]))
                        {
                            num++;
                        }
                    }
            ...
            for (int i = 0; i < equipList.Count; i++)
            {
                ...
                //强化登录（List长度13， 13个位置）
                int qianghuaLv = unit.GetComponent<BagComponentServer>().GetQiangHuaLevel(itemCof.ItemSubType);
            ...
            //家园守护
            List<PropertyValue> shouhuPros = unit.GetComponent<PetComponentServer>().GetPetShouHuPro();
            ...
            //---------------
            NumericComponentServer numericComponent_1 = new NumericComponentServer();
            ...
            if (rank)
            {
                //unit.GetComponent<UserInfoComponentServer>().UpdateRankInfo();
            }
