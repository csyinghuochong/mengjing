namespace ET.Server
{
    ...
    public class C2M_ItemFumoUseHandler: MessageLocationHandler<Unit, C2M_ItemFumoUseRequest, M2C_ItemFumoUseResponse>
    {
        protected override async ETTask Run(Unit unit, C2M_ItemFumoUseRequest request, M2C_ItemFumoUseResponse response)
        {
            ...
            unit.GetComponent<TaskComponentServer>().TriggerTaskEvent(TaskTargetType.FuMoQulity_41, itemConfig.ItemQuality, 1);
            unit.GetComponent<TaskComponentServer>().TriggerTaskCountryEvent(TaskTargetType.FuMoQulity_41, itemConfig.ItemQuality, 1);
