...
namespace ET.Server
{
    ...
    public class C2M_MakeEquipHandler : MessageLocationHandler<Unit, C2M_MakeEquipRequest, M2C_MakeEquipResponse>
    {
        protected override async ETTask Run(Unit unit, C2M_MakeEquipRequest request, M2C_MakeEquipResponse response)
        {
            ...
            unit.GetComponent<UserInfoComponentS>().UpdateRoleMoneySub(UserDataType.Gold, (equipMakeConfig.MakeNeedGold * -1).ToString(), true, ItemGetWay.SkillMake);
            ...
            if (equipMakeConfig.MakeSuccesPro >= rate)
            {
                ...
                unit.GetComponent<BagComponentS>().OnAddItemData(rewardItems, unit.GetComponent<UserInfoComponentS>().UserInfo.Name, $"{ItemGetWay.SkillMake}_{request.Plan}_{TimeHelper.ServerNow()}");       //传入制造装备和制造玩家的ID
            ...
            unit.GetComponent<TaskComponentS>().TriggerTaskCountryEvent(TaskTargetType.MakeNumber_12, 0, 1);
            unit.GetComponent<TaskComponentS>().TriggerTaskEvent(TaskTargetType.MakeQulityNumber_29, itemConfig.ItemQuality, 1);
            unit.GetComponent<TaskComponentS>().TriggerTaskCountryEvent(TaskTargetType.MakeQulityNumber_29, itemConfig.ItemQuality, 1);
