    ...
    public class C2M_ItemOperateWearHandler: MessageLocationHandler<Unit, C2M_ItemOperateWearRequest, M2C_ItemOperateWearResponse>
    {
        protected override async ETTask Run(Unit unit, C2M_ItemOperateWearRequest request, M2C_ItemOperateWearResponse response)
        {
             ...
             if (itemConfig.ItemSubType == 9)   //充值达到一定额度开启宝箱获得道具
             {
                 ...
                 if (unit.GetComponent<NumericComponentServer>().GetAsLong(NumericType.RechargeNumber) < long.Parse(itemPar[0]))
                 {
...
KK
             ...
             {
                 int openDay = UnitCacheHelper.GetOpenServerDay(unit.Zone());
                 if (openDay <= 1)
                 {
                     response.Error = ErrorCode.ERR_ItemNoUseTime;
                     return;
             ...
             if (itemConfig.ItemSubType == 138)
             {
                 ...
                 if (unit.GetComponent<NumericComponentServer>().GetAsInt(NumericType.TeamDungeonTimes) <= 0)
                 {
     ...
     if (bagComponent.OnCostItemData(useBagInfo, ItemLocType.ItemLocBag, costNumber))
     {
         ...
         switch (itemConfig.ItemSubType)
         {
             ...
             //回城卷轴[返回另外一个副本场景]
             case 4:
                 if (unit.Root().GetComponent<MapComponent>().SceneTypeEnum == (int)SceneTypeEnum.LocalDungeon)
                 {
             ...
             case 6:
                 ...
                 if (dropId > 0)
                 {
                     ...
                     bagComponent.OnAddItemData(droplist, string.Empty, $"{ItemGetWay.ItemBox_6}_{TimeHelper.ServerNow()}_{itemConfig.Id}");
                 }
                 break;
             //兑换：
             case 8:
             ...
             case 15:    //附魔道具
                 ...
                 break;
             case 16: //附魔技能
                 UserInfoComponentServer.UserInfo.MakeList.Add(int.Parse(itemConfig.ItemUsePar));
                 break;
             //使用技能
             case 101:
                 break;
             ...
             case 102:
                 ...
                 unit.GetComponent<PetComponentServer>().OnAddPet(int.Parse(getway[0]), int.Parse(itemConfig.ItemUsePar), 0, useBagInfo.FuLing);
                 break;
             //随机宠物蛋
             case 103:
             ...
             case 114: //宝石
                 break;
             case 115://宠物皮肤激活道具
                 unit.GetComponent<PetComponentServer>().OnUnlockSkin(itemConfig.ItemUsePar);
                 break;
             case 116:   //角色洗点
             ...
             case 119:   //宠物成长
                 break;
             case 120://120 冒险积分
                 unit.GetComponent<NumericComponentServer>().ApplyChange(null, NumericType.MaoXianExp, int.Parse(itemConfig.ItemUsePar), 0);
                 break;
             case 121: //鉴定符
                 break;
             case 122:   //宠物技能书
                 break;
             case 123:   //宠物扩展工具
                 int petnumer = unit.GetComponent<NumericComponentServer>().GetAsInt(NumericType.PetExtendNumber);
                 unit.GetComponent<NumericComponentServer>().SetEvent( NumericType.PetExtendNumber, petnumer + 1,true);
                 break;
             case 124: //仓库扩展工具
                 int cangkuNumber = unit.GetComponent<NumericComponentServer>().GetAsInt(NumericType.CangKuNumber);
                 unit.GetComponent<NumericComponentServer>().SetEvent(NumericType.CangKuNumber, cangkuNumber + 1,true);
                 break;
             case 125://坐骑获取
                 ...
                 {
                     unit.GetComponent<NumericComponentServer>().SetEvent(NumericType.HorseFightID, hourseId, true);
                 }
             ...
             case 132:
                 ...
                 unit.GetComponent<NumericComponentServer>().ApplyChange(null, NumericType.SeasonBossRefreshTime, -1 * reduceTime, 0);
                 break;
             case 133:
             case 134:
                 break;
             ...
             case 138:
                 // 增加副本次数
                 int teamTime = unit.GetComponent<NumericComponentServer>().GetAsInt(NumericType.TeamDungeonTimes);
                 unit.GetComponent<NumericComponentServer>().SetEvent(NumericType.TeamDungeonTimes, teamTime - 1, true);
                 break;
             case 139:
                 //增加背包格子
 ...
 if (request.OperateType == 3)
 {
     ...
     if (weizhi == (int)ItemSubTypeEnum.Wuqi)
     {
         ...
         unit.GetComponent<NumericComponentServer>().SetEvent(NumericType.Now_Weapon, useBagInfo.ItemID, true);
         unit.GetComponent<NumericComponentServer>().SetEvent(NumericType.WearWeaponFisrt, 1, true);
     }
 }
 ...
 if (request.OperateType == 4)
 {
     ...
     if (weizhi == (int)ItemSubTypeEnum.Wuqi)
     {
         ...
         unit.GetComponent<NumericComponentServer>().SetEvent(NumericType.Now_Weapon, 0, true);
     }
 }
