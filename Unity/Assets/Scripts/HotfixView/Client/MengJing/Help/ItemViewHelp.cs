...
namespace ET.Client
{
    public static class ItemViewHelp
    {
        ...
        public static int ShowXiLianAttribute(List<ItemInfo> equipItemList, ItemInfo baginfo, GameObject propertyGO,
        GameObject parentGO)
        {
            int properShowNum = 0;
            ItemConfig itemConfig = ItemConfigCategory.Instance.Get(baginfo.ItemID);
            ...
            if (equip_Hp != 0)
            {
                ...
                if (ifHideProperty)
                {
                    GameObject newGo = ShowPropertyText(textShow, "1", propertyGO, parentGO);
                    ShowProgressBar(hidePropertyValue, equipConfig.HideMax, newGo);
            ...
            if (baginfo.XiLianHideTeShuProLists != null)
            {
                for (int i = 0; i < baginfo.XiLianHideTeShuProLists.Count; i++)
                {
                        ...
                        nowType != NumericType.Base_MaxDef_Base && nowType != NumericType.Base_MaxAdf_Base)
                    {
                        ...
                        GameObject newGo = ShowPropertyText(proStr, showColor, propertyGO, parentGO);
                        ...
                        ShowProgressBar(hidePropertyValue, NumericHelp.NumericValueSaveType(hideProListConfig.PropertyType, propertyValueMax), newGo);
            ...
            if (baginfo.HideSkillLists != null)
            {
                ...
                for (int i = 0; i < baginfo.HideSkillLists.Count; i++)
                {
                    ...
                    SkillConfig skillCof = SkillConfigCategory.Instance.Get(skillID);
                    string proStr = LanguageComponent.Instance.LoadLocalization(skillTip) + skillCof.SkillName;
                    GameObject newGo = ShowPropertyText(proStr, "2", propertyGO, parentGO);
                    ShowProgressBar(1, 1, newGo);
        ...
        public static void ShowProgressBar(float now, float max, GameObject proItem)
        {
