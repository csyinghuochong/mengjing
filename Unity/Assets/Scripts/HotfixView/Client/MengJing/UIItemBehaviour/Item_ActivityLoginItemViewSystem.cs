...
namespace ET.Client
{
    ...
    public static partial class Scroll_Item_ActivityLoginItemSystem
    {
        ...
        public static async ETTask OnBtn_Receive(this Scroll_Item_ActivityLoginItem self)
        {
            ...
            if (userInfo.Lv < 10)
            {
                FlyTipComponent.Instance.SpawnFlyTipDi(GameSettingLanguge.LoadLocalization("10级才能领取！"));
                return;
            }

            if (!self.CanReceive(self.ActivityConfig.Id))
            {
                FlyTipComponent.Instance.SpawnFlyTipDi(GameSettingLanguge.LoadLocalization("未达到领取条件！"));
                return;
            }
            ...
            {
                FlyTipComponent.Instance.SpawnFlyTipDi(GameSettingLanguge.LoadLocalization("今天的奖励已领取"));
                return;
            }
