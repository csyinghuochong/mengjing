...
namespace ET.Client
{
    ...
    public static class DlgJiaYuanPlanWatchSystem
    {
        ...
        public static async ETTask OnInitUI(this DlgJiaYuanPlanWatch self)
        {
            ...
            self.View.E_Text_Desc_1Text.text = $"当前阶段: {ET.JiaYuanHelper.GetPlanStageName(stage)}";

            long nextTime = ET.JiaYuanHelper.GetNextStateTime(unit.ConfigId, StartTime);
            self.View.E_Text_Desc_2Text.text = $"下一阶段: {ET.JiaYuanHelper.TimeToShow(TimeInfo.Instance.ToDateTime(nextTime).ToString("f"))}";
            ...
            for (int i = 0; i < response.JiaYuanRecord.Count; i++)
            {
                ...
                gatherrecode += $"{gatherTime} {response.JiaYuanRecord[i].PlayerName}收获一次 \n";
            }
