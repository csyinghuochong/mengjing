    ...
    public static partial class ES_ActivitySingInFreeSystem
    {
        ...
        private static void Awake(this ES_ActivitySingInFree self, Transform transform)
        {
            ...
            self.E_Reward1EventTrigger.RegisterEvent(EventTriggerType.PointerUp,
                (pdata) => { self.EndDrag(ConfigData.TotalSignRewards.ToList()[0].Key, pdata as PointerEventData).Coroutine(); });
            self.E_Reward2EventTrigger.RegisterEvent(EventTriggerType.PointerUp,
                (pdata) => { self.EndDrag(ConfigData.TotalSignRewards.ToList()[1].Key, pdata as PointerEventData).Coroutine(); });
            self.E_Reward3EventTrigger.RegisterEvent(EventTriggerType.PointerUp,
                (pdata) => { self.EndDrag(ConfigData.TotalSignRewards.ToList()[2].Key, pdata as PointerEventData).Coroutine(); });
            self.E_Reward4EventTrigger.RegisterEvent(EventTriggerType.PointerUp,
                (pdata) => { self.EndDrag(ConfigData.TotalSignRewards.ToList()[3].Key, pdata as PointerEventData).Coroutine(); });
        ...
        private static void OnInitUI(this ES_ActivitySingInFree self)
        {
            ...
            using (zstring.Block())
            {
                string[] itemInfo = ConfigData.TotalSignRewards.ToList()[0].Value.Split(';');
                ...
                self.E_Reward1EventTrigger.transform.Find("Text").GetComponent<Text>().text =
                        zstring.Format("累计签到{0}天", ConfigData.TotalSignRewards.ToList()[0].Key);

                itemInfo = ConfigData.TotalSignRewards.ToList()[1].Value.Split(';');
                ...
                self.E_Reward2EventTrigger.transform.Find("Text").GetComponent<Text>().text =
                        zstring.Format("累计签到{0}天", ConfigData.TotalSignRewards.ToList()[1].Key);

                itemInfo = ConfigData.TotalSignRewards.ToList()[2].Value.Split(';');
                ...
                self.E_Reward3EventTrigger.transform.Find("Text").GetComponent<Text>().text =
                        zstring.Format("累计签到{0}天", ConfigData.TotalSignRewards.ToList()[2].Key);

                itemInfo = ConfigData.TotalSignRewards.ToList()[3].Value.Split(';');
                ...
                self.E_Reward4EventTrigger.transform.Find("Text").GetComponent<Text>().text =
                        zstring.Format("累计签到{0}天", ConfigData.TotalSignRewards.ToList()[3].Key);
            }
        ...
        private static async ETTask EndDrag(this ES_ActivitySingInFree self, int day, PointerEventData pdata)
        {
            ActivityComponentC activityComponent = self.Root().GetComponent<ActivityComponentC>();
            if (activityComponent.TotalSignRewardsList.Contains(day))
            {
                FlyTipComponent.Instance.ShowFlyTip("已领取");
                return;
            }

            if (activityComponent.TotalSignNumber < day)
            {
                FlyTipComponent.Instance.ShowFlyTip("未达到领取条件");
                return;
            }

            int error = await ActivityNetHelper.ActivityTotalSignReceive(self.Root(), 0, day);
            if (error != ErrorCode.ERR_Success)
            {
                return;
            }
        ...
        private static void UpdateProgress(this ES_ActivitySingInFree self)
        {
            ...
            self.E_Reward1EventTrigger.transform.Find("LingQu").gameObject
                    .SetActive(activityComponent.TotalSignRewardsList.Contains(ConfigData.TotalSignRewards.ToList()[0].Key));
            self.E_Reward2EventTrigger.transform.Find("LingQu").gameObject
                    .SetActive(activityComponent.TotalSignRewardsList.Contains(ConfigData.TotalSignRewards.ToList()[1].Key));
            self.E_Reward3EventTrigger.transform.Find("LingQu").gameObject
                    .SetActive(activityComponent.TotalSignRewardsList.Contains(ConfigData.TotalSignRewards.ToList()[2].Key));
            self.E_Reward4EventTrigger.transform.Find("LingQu").gameObject
                    .SetActive(activityComponent.TotalSignRewardsList.Contains(ConfigData.TotalSignRewards.ToList()[3].Key));
        }
    }
}